/*
 * @Author: xiaoyu.ren
 * @Date: 2019-01-18 19:39:08
 * @LastEditors: xiaoyu.ren
 * @LastEditTime: 2020-05-26 11:25:10
 * @Description:
 */
import Vue from "vue";
...
//v-dictType: 数据字典 存储vuex
Vue.directive("dictType", {
  bind(el, binding, vnode, oldVnode) {
    var dictTypeArr = binding.value;
    for (let i = 0, len = dictTypeArr.length; i < len; i++) {
      //vuex中取不到该字典项，则通过服务器请求获取
      if (
        !Store.state["common"][dictTypeArr[i]] ||
        Store.state["common"][dictTypeArr[i]].length == 0
      ) {
        getDictByType({
          dictType: dictTypeArr.join(",") //dictTypeArr:["dictType1","dictType2"]
        }).then(res => {
          if (res["code"] == 200) {
            //获取到字典数据，存储到vuex中
            Store.state["common"] = Object.assign({}, Store.state["common"], {
              [dictTypeArr[i]]: res.data[dictTypeArr[i]]
            });
            //   Store.state["common"][dictTypeArr[i]] = res.data[dictTypeArr[i]];
          }
        });
      }
    }
